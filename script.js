...
const data = JSON.parse(json); //Массив с данными
let selectedRow = null;
let currentPage = 0;
let sortColumn = ""; //Столбец, по которому отсортирована таблица
const pageRows = 10; //Число записей на странице

fillTable(currentPage);

//Если необходимо(данные не вмещаются в одну страницу), отрисовываем пагинацию
...
function fillTable(page) {
  ...
  for (let i = pageRows * page; i < pageRows * (page + 1); i++) {
    //для всех записей текущей страницы
    //создаем строку только с выбранными пользователем столбцами
    const tr = document.createElement("tr");
    if (checkboxes[0].checked) {
      const firstName = document.createElement("td");
      firstName.textContent = data[i].name.firstName;
      tr.append(firstName);
    }
    if (checkboxes[1].checked) {
      const lastName = document.createElement("td");
      lastName.textContent = data[i].name.lastName;
      tr.append(lastName);
    }
    if (checkboxes[2].checked) {
      const about = document.createElement("td");
      about.className = "about";
      about.textContent = data[i].about;
      tr.append(about);
    }
    if (checkboxes[3].checked) {
      const eyeColor = document.createElement("td");
      eyeColor.style.backgroundColor = data[i].eyeColor;
      tr.append(eyeColor);
    }
    tr.onclick = showForm;
    //добавляем строку в фрагмент
    fragment.append(tr);
  }
  //вставляем фрагмент в таблицу
  new_tbody.append(fragment);
  //заменяем предыдущее состояние таблицы новым
  tbody.parentNode.replaceChild(new_tbody, tbody);
}

function sortTable(column) {
  //если предыдущая сортировка выполнялась по этому же столбцу, сортируем в другом порядке
  if (column.id === sortColumn) {
    sortDesc(column.id);
    //сбрасываем значение, потому что при следующей сортировке по этому же столбцу должен быть снова изначальный порядок
    sortColumn = "";
  } else {
    sortAsc(column.id);
    sortColumn = column.id;
  }
  //меняем строки таблицы на отсортированные
  fillTable(currentPage);
}
...
function sortAsc(field) {
  if (field === "firstName") {
    data.sort((a, b) => (a.name.firstName > b.name.firstName ? 1 : -1));
  } else if (field === "lastName") {
    data.sort((a, b) => (a.name.lastName > b.name.lastName ? 1 : -1));
  } else if (field === "about") {
    data.sort((a, b) => (a.about > b.about ? 1 : -1));
  } else if (field === "eyeColor") {
    data.sort((a, b) => (a.eyeColor > b.eyeColor ? 1 : -1));
  }
}
...
function sortDesc(field) {
  if (field === "firstName") {
    data.sort((a, b) => (b.name.firstName > a.name.firstName ? 1 : -1));
  } else if (field === "lastName") {
    data.sort((a, b) => (b.name.lastName > a.name.lastName ? 1 : -1));
  } else if (field === "about") {
    data.sort((a, b) => (b.about > a.about ? 1 : -1));
  } else if (field === "eyeColor") {
    data.sort((a, b) => (b.eyeColor > a.eyeColor ? 1 : -1));
  }
}
...
function toggleColumn() {
  ...
  //если не отмечен ни один столбец, скрываем таблицу
  if (checked.length === 0) {
    table.style.display = "none";
  } else {
    table.style.display = "flex";
    hideForm();
    //в шапку таблицы записываем только выбранные столбцы
    let checkboxes = document.querySelectorAll("input[type=checkbox]");
    let thead = document.querySelector("thead");
    const old_tr = document.getElementById("table-head");
    ...
    if (checkboxes[0].checked) {
      ...
      firstName.addEventListener("click", () => sortTable(firstName));
      new_tr.append(firstName);
    }
    if (checkboxes[1].checked) {
      ...
      lastName.addEventListener("click", () => sortTable(lastName));
      new_tr.append(lastName);
    }
    if (checkboxes[2].checked) {
      const about = document.createElement("th");
      about.textContent = "Описание";
      about.id = "about";
      about.addEventListener("click", () => sortTable(about));
      new_tr.append(about);
    }
    if (checkboxes[3].checked) {
      const eyeColor = document.createElement("th");
      eyeColor.textContent = "Цвет глаз";
      eyeColor.className = "color";
      eyeColor.id = "eyeColor";
      eyeColor.addEventListener("click", () => sortTable(eyeColor));
      new_tr.append(eyeColor);
    }
    //заменяем старую шапку таблицы на новую
    thead.replaceChild(new_tr, old_tr);
    new_tr.id = "table-head";

    //меняем строки таблицы на отфильтрованные
    fillTable(currentPage);
  }
}
...
function fillPagination() {
  ...
  //число страниц
  const pagesCount = Math.ceil(data.length / pageRows);
  //для каждой страницы создаем кнопку
...
//заполнение формы информацией из таблицы
function fillForm(e) {
  let checkboxes = document.querySelectorAll("input[type=checkbox]");
  ...
  for (let i = 0; i < inputs.length - 1; i++) {
    ...
    //позволяем менять в форме только ту информацию, которая показана в таблице
    if (checkboxes[i].checked) {
      inputs[i].style.display = "block";
      label.style.display = "block";
      inputs[i].value = selectedRow.querySelectorAll("td")[td_num].textContent;
      td_num++;
    } else {
  ...
  //заполнение информации о цвете глаз
  const label = document.querySelector(
    `label[for=${inputs[inputs.length - 1].id}]`
  );
  if (checkboxes[inputs.length - 1].checked) {
    label.style.display = "block";
    inputs[inputs.length - 1].style.display = "block";
    inputs[inputs.length - 1].value =
      selectedRow.querySelectorAll("td")[td_num].style.backgroundColor;
  } else {
...
function submitForm(e) {
  ...
  //вносим цвет глаз в таблицу
  cells[inputs.length - 1].style.backgroundColor =
    inputs[inputs.length - 1].value;
}
